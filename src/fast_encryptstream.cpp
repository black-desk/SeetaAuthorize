...
namespace orz
{
  static void encrypt(unsigned char *buf, int64_t len, int64_t key) {
    ...
    for (int64_t i = 0; i < nstep; i++) { ptr [i] = ptr [i] ^ key; }
  }
  ...
  int64_t Fast_EncryptInputStream::read(char *buffer, int64_t len) {
    ...
    int nreads = m_in->read(
      buffer + buffer_offset, len - buffer_offset - nremain);
    if (nreads <= 0) {
      m_eof = true;
      return buffer_offset;
    }
    ...
    if (nreads < len - buffer_offset - nremain) { m_eof = true; }

    decrypt((unsigned char *)(buffer + buffer_offset), nreads, m_cskey);
    buffer_offset += nreads;

    if (m_eof) { return buffer_offset; }

    if (nremain > 0) {
      m_master_datalen = m_in->read(
        reinterpret_cast<char *>(m_master), AES_BLOCKLEN);
      if (m_master_datalen <= 0) {
        ...
        if (m_master_datalen != AES_BLOCKLEN) {
          orz::Log(orz::ERROR)
            << "mode file read format is error!" << orz::crash;
          return 0;
        }
  ...
  Fast_EncryptInputStream::Fast_EncryptInputStream(
    ...
    // if(nret != 0) {
    //     orz::Log(orz::FATAL) << "AES_set_decrypt_key failed:" << nret <<
    //     orz::crash;
    // }
  }
  ...
  int64_t Fast_EncryptOutputStream::write(const char *buffer, int64_t len) {
    ...
    while (nwrite < len) {
      if (AES_BLOCKLEN - m_master_datalen >= len - nwrite) {
        ...
        if (
          m_out->write(reinterpret_cast<const char *>(m_master), AES_BLOCKLEN)
          != AES_BLOCKLEN) {
          orz::Log(orz::ERROR)
            << "Fast_EncryptOutputStream write failed!" << orz::crash;
          return 0;
        }
  ...
  void Fast_EncryptOutputStream::flush() {
    ...
    if (
      m_out->write(reinterpret_cast<const char *>(buf), AES_BLOCKLEN)
      != AES_BLOCKLEN) {
      orz::Log(orz::ERROR) << "Fast_EncryptOutputStream write failed!"
                           << orz::crash;
      return;
    }
    m_master_datalen = 0;
  ...
  Fast_EncryptOutputStream::Fast_EncryptOutputStream(
    std::shared_ptr<OutputStream> out, const std::string &key) :
  ...
  Fast_EncryptOutputStream::~Fast_EncryptOutputStream() { flush(); }
