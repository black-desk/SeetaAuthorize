...
namespace orz
{
  ...
  int CstaModelFileInputStream::get_encrypt_type() { return m_encrypt_type; }
  ...
  CstaModelFileInputStream::CstaModelFileInputStream(
    ...
    if (mask == 0x19910929) {
      ...
      orz::Log(orz::ERROR) << "model file:" << file << " is not supported!"
                           << orz::crash;
      /*
      if( key.length() < 1 )
      {
      ...
      m_encrypt_type = 1;
      std::shared_ptr<InputStream> csta_file = std::shared_ptr<InputStream>( new
      FileInputStream( file ) ); std::shared_ptr<InputStream> csta =
      std::shared_ptr<InputStream>( new CstaInputStream( csta_file ) ); m_in =
      std::shared_ptr<InputStream>( new EncryptInputStream( csta, key ) );
      */
    }

    else if (mask == 0x74736166) {
      if (key.length() < 1) {
        orz::Log(orz::ERROR) << "paramter key is empty!" << orz::crash;
      }

      m_encrypt_type                         = 0;
      std::shared_ptr<InputStream> csta_file = std::shared_ptr<InputStream>(
        new FileInputStream(file));
      std::shared_ptr<InputStream> csta = std::shared_ptr<InputStream>(
        new Fast_CstaInputStream(csta_file));
      m_in = std::shared_ptr<InputStream>(
        new Fast_EncryptInputStream(csta, key));
  ...
  orz::jug CstaModelFileInputStream::read_jug() {
    ...
    if (m_istxt) {
      ...
      if (nfind >= 0) {
        ...
        if (nfind >= 0) { strpath = m_file.substr(0, nfind + 1); }
      }
  ...
  orz::jug CstaModelFileInputStream::read_jug(
    std::shared_ptr<InputStream> in, const std::string &key) {
    unsigned int mask    = 0;
    int64_t      nreads  = 0;
    bool         istxt   = false;
    int          noffset = 0;
    ...
    if (mask == 0x19910929) {
      ...
      */
      ...
      noffset = 4;
      ifile   = std::shared_ptr<InputStream>(
        new Fast_EncryptInputStream(in, key));
  ...
  int CstaModelFileOutputStream::get_encrypt_type() { return m_encrypt_type; }
  ...
  CstaModelFileOutputStream::CstaModelFileOutputStream(
    ...
    if (m_istxt) {
      ...
      if (key.length() < 1) {
        ...
        if (encrypt_type == 0) {
          std::shared_ptr<OutputStream> csta_file =
            std::shared_ptr<OutputStream>(new FileOutputStream(file));
          std::shared_ptr<OutputStream> csta = std::shared_ptr<OutputStream>(
            new Fast_CstaOutputStream(csta_file));
          m_out = std::shared_ptr<OutputStream>(
            new Fast_EncryptOutputStream(csta, key));
        ...
        /*
                            ...
                            if( encrypt_type == 1 )
                            {
                                std::shared_ptr<OutputStream> csta_file =
           std::shared_ptr<OutputStream>( new FileOutputStream( file ) );
                                std::shared_ptr<OutputStream> csta =
           std::shared_ptr<OutputStream>( new CstaOutputStream( csta_file ) );
                                m_out = std::shared_ptr<OutputStream>( new
           EncryptOutputStream( csta, key ) );

                                //std::cout << "encrypt_type:" << encrypt_type
           << std::endl;
                            }
        */
        else {
          orz::Log(orz::FATAL)
            << "not support encrypt type:" << encrypt_type << orz::crash;
        }
      }
    }
  }
